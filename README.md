...
- **Изображения чата** хранятся вне `public/` (по умолчанию рядом с БД в `chat-uploads`, либо `CHAT_UPLOADS_PATH`). Каталог нужно сохранять при деплое и бэкапить вместе с БД; выдача только владельцу сессии через `/api/chat/attachments/...`.
