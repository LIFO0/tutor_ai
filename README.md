...
| `src/app/api/auth/*` | Регистрация, логин, выход, OAuth Яндекса, опционально тестовый вход |
...
Файл `.env.local` не коммитится. Полный перечень и комментарии — в [`.env.local.example`](./.env.local.example): ключи YandexGPT, `JWT_SECRET`, OAuth Яндекса, опционально `DATABASE_PATH`, `ALLOW_TEST_LOGIN` (только для демо; в проде держите выключенным).
...
- **Нагрузка на API:** для одного процесса Node добавлен простой **лимит по IP** на `POST` к `/api/auth/login`, `/api/auth/register` и `/api/auth/test`. При нескольких воркерах или за reverse proxy смотрите также лимиты на уровне nginx/Caddy.
